...
class RailApproachActionServer(Node):
    def __init__(self):
        ...
        self.declare_parameter('pulse_linear_speed', 0.03)
        self.declare_parameter('pulse_forward_sec', 0.2)

        self.declare_parameter('commit_linear_speed', 0.02)
        self.declare_parameter('commit_forward_sec', 2.0)

        self.declare_parameter('angular_speed', 0.08)
        self.declare_parameter('rail_timeout_sec', 0.5)
        self.declare_parameter('close_bbox_area_ratio', 0.18)
        ...
        self.commit_linear_speed = float(
            self.get_parameter('commit_linear_speed').value
        )
        self.commit_forward_sec = float(
            self.get_parameter('commit_forward_sec').value
        )

        self.angular_speed = float(self.get_parameter('angular_speed').value)
        self.rail_timeout_sec = float(self.get_parameter('rail_timeout_sec').value)
        self.close_bbox_area_ratio = float(
            self.get_parameter('close_bbox_area_ratio').value
        )
    ...
    def get_turn_speed(self, angle_error):
    ...
    def execute_callback(self, goal_handle):
        ...
        while rclpy.ok():
            ...
            bbox_area_ratio = float(
                self.get_field(rail, 'rail_bbox_area_ratio', 0.0)
            )
            ...
            close_enough = bbox_area_ratio >= self.close_bbox_area_ratio
            ...
            if state == 'ALIGN':
                if not angle_ok:
                    cmd.angular.z = self.get_turn_speed(angle_error)
                elif not x_ok:
                    cmd.angular.z = 0.0
                elif close_enough:
                    state = 'COMMIT_FORWARD'
                    commit_end_time = now + self.commit_forward_sec
                    cmd.linear.x = self.commit_linear_speed
                else:
                    state = 'PULSE_FORWARD'
                    pulse_end_time = now + self.pulse_forward_sec
                    cmd.linear.x = self.pulse_linear_speed

            elif state == 'PULSE_FORWARD':
                if not angle_ok or not x_ok:
                    state = 'ALIGN'
                    pulse_end_time = None
                elif close_enough:
                    state = 'COMMIT_FORWARD'
            ...
            elif state == 'COMMIT_FORWARD':
                if commit_end_time is not None and now < commit_end_time:
                    cmd.linear.x = self.commit_linear_speed
                else:
                    self.stop_robot()
            ...
            self.get_logger().info(
                ...
                f'dist={distance}, '
                f'bbox_area={bbox_area_ratio:.3f}/{self.close_bbox_area_ratio:.3f}, '
