...
class ArucoDockerNode(Node):
    ...
    def toggle_callback(self, request, response):
        ...
        if self.is_docking_active:
            self.last_pose_time = self.get_clock().now() 
            self.get_logger().info("▶️ 도킹 모드가 활성화되었습니다. (위치 및 각도 정렬 중...)")
    ...
    def check_timeout(self):
        ...
        if elapsed_time > 0.5:
            self.get_logger().warn("⚠️ ArUco 마커 소실! 회전이 너무 빠르거나 카메라를 벗어났습니다. 정지합니다.")
            self.cmd_pub.publish(Twist())
            self.is_docking_active = False
    ...
    def pose_callback(self, msg):
        ...
        if abs(error_z) <= self.tol_z and abs(error_x) <= self.tol_x and abs(error_yaw) <= self.tol_yaw:
            self.get_logger().info("✅ 도킹 완료! 목표 위치와 각도에 완벽히 도달했습니다.")
