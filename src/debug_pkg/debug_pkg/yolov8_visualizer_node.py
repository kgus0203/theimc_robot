...
class Yolov8VisualizerNode(LifecycleNode):
    ...
    def on_activate(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.image_sub = message_filters.Subscriber(
            self, Image, "image_raw", qos_profile=self.image_qos_profile) # image_raw로 수정하기
    ...
    def draw_box(self, cv_image: np.array, detection: Detection, color: Tuple[int]) -> np.array:
        ...
        # draw center position red dot 
        if label == "rail_start":
            center_pt = (int(box_msg.center.position.x), int(box_msg.center.position.y))
            cv2.circle(cv_image, center_pt, 5, (0, 0, 255), -1)  # 빨간색(BGR: (0,0,255)), 반지름=5, 채워진 원
